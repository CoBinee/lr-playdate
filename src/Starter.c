...
void StarterLoad(void)
{
    ...
    // アクタの登録
    struct Starter *starter = (struct Starter *)ActorLoad((ActorFunction)StarterLoop, kTitlePriorityDemo);
    if (starter == NULL) {
...
static void StarterDraw(struct Starter *starter)
{
    ...
    {
        char *text;
        playdate->system->formatString(&text, "$%d", ApplicationGetScore());
        int width = IocsGetTextWidth(kIocsFontGame, text);
        playdate->graphics->setDrawMode(kDrawModeFillBlack);
        playdate->graphics->drawText(text, strlen(text), kUTF8Encoding, (LCD_COLUMNS - width) / 2, 4);
